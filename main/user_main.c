...
void example_espnow_data_prepare(example_espnow_send_param_t *send_param)
{
  ...
  //  ESP_LOGW(TAGT, "prepare data sent:  %s", buf->payload);
...
static void example_espnow_task(void *pvParameter)
{
    ...
    ESP_LOGW(TAGT, "send data - intrare in task");
        ...
        switch (evt.id) {
            case EXAMPLE_ESPNOW_SEND_CB:
            {
                ...
                if (is_broadcast && (send_param->broadcast == false)) {
                	ESP_LOGW(TAGT, "Broadcast break - warning  GALBEN");
                	ESP_LOGE(TAGT, "Broadcast break -error ROSU");
                	                    break;
                }
                ...
                if (!is_broadcast) {
                    ...
                    for (t = 0; t < peer_number->total_num; t++)
                    {
                        ...
                        ESP_LOGE(TAG, "Adresa peer %d:  "MACSTR" ", t,  MAC2STR(peer_info->peer_addr));
                        // ESP_ERROR_CHECK( esp_now_fetch_peer(0,peer_info));
                    }
                    // ESP_LOGW(TAGT, "GET PEER NUMBER - %d",peer_number->total_num);
*/
            ...
            case EXAMPLE_ESPNOW_RECV_CB:
            {
                ...
                ESP_LOGE(TAGT, "inainte de  - flag 1");
                free(recv_cb->data);
                if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
                    ...
                    if (send_param->state == 0) {
                        send_param->state = 1;
                        ESP_LOGE(TAGT, "a primit broadcast data - flag 1");
                    }
                    ...
                    if (recv_state == 1) {
                        ...
                        if (send_param->unicast == false && send_param->magic >= recv_magic) {
                    	    ...
                    	    ESP_LOGW(TAGT, "send data in receive");
                    	    ESP_LOGE(TAG, "send data to "MACSTR"", MAC2STR(send_param->dest_mac));
                            ...
                            if (esp_now_send(send_param->dest_mac, send_param->buffer, send_param->len) != ESP_OK) {
                            	...
                            	ESP_LOGW(TAGT, "send data in receive - else");
...
static esp_err_t example_espnow_init(void)
{
    ...
    ESP_ERROR_CHECK( esp_now_add_peer(peer) );
    ESP_LOGE(TAG, "Adresa peer :  "MACSTR" ",  MAC2STR(peer->peer_addr));
    free(peer);
    ...
    //ESP_LOGW(TAGT, "init data sent:  %.20s", (uint8_t*)(send_param->buffer));
  //  ESP_LOGW(TAGT, "init data sent:  %.d", (int)send_param->buffer);
    char *TmpStr = "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT";
    //strncpy(TmpStr,(char *)(send_param->buffer),20);
    example_espnow_data_t *buf = (example_espnow_data_t *)send_param->buffer;
    ESP_LOGW(TAGT, "init payload data sent:  %.10s", buf->payload);
  //  ESP_LOGW(TAGT, "init data sent:  %s", TmpStr);
    ESP_LOGW(TAGT, "prepare data sent:  %c", buf->payload[0]);

   memcpy(send_param->buffer, TmpStr, 20);
   ESP_LOGW(TAGT, "init buffer data sent:  %.10s", send_param->buffer);
